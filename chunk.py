...
class Chunk:
    ...
    def update_mesh(self):
        ...
        def add_face(face):
            vertex_positions = current_block_type.vertex_positions[face].copy()

            for vertex in range(4):
                vertex_positions[vertex * 3] += x
                vertex_positions[vertex * 3 + 1] += y
                vertex_positions[vertex * 3 + 2] += z

            self.mesh_vertex_positions.extend(vertex_positions)

            indices = [0, 1, 2, 0, 2, 3]
            for index in range(6):
                indices[index] += self.mesh_index_counter

            self.mesh_indices.extend(indices)
            self.mesh_index_counter += 4

            self.mesh_tex_coords.extend(current_block_type.tex_coords[face])
            self.mesh_shading_values.extend(
                current_block_type.shading_values[face]
            )
        ...
        for local_x in range(CHUNK_WIDTH):
            for local_y in range(CHUNK_HEIGHT):
                for local_z in range(CHUNK_LENGTH):
                    ...
                    current_block_type = self.world.block_types[block_number]
                    x, y, z = (
        ...
        gl.glBufferData(
            ...
            (gl.GLfloat * len(self.mesh_shading_values))(
                *self.mesh_shading_values
            ),
            gl.GL_STATIC_DRAW,
        )
